...
function ConsulterRecette({ navigation, marmitonToDisplay, clickModifierParent }) {

    const [isVisible, setIsVisible] = useState(false)
    const [operator, setOperator] = useState(null)
    const [nombre, setNombre] = useState(1)
    const [ingredients, setIngredients] = useState(marmitonToDisplay.ingredients)
    ...
    let ingredientTable = marmitonToDisplay.ingredients.map((ing, i) => {
    ...
    let preparationTable = marmitonToDisplay.preparation.map((etape, i) => {
